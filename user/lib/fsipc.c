...
//  < 0 on failure.
//
static int getFsNumber(const char *path) {
	char tmpName[20];
	strcpy(tmpName,path);
	tmpName[ROOTDIR_NAMELEN] = 0;
	if (strcmp(tmpName,"/root1") == 0) {
		return NORMAL_FS_NUMBER;
	} else if (strcmp(tmpName,"/root2") == 0) {
		return FAT32_FS_NUMBER;
	}
	return -E_NOT_FOUND;
}


static int fsipc(u_int type, void *fsreq, void *dstva, u_int *perm,u_int whichfs) {
	u_int whom;
	// Our file system server must be the 2nd env.
	ipc_send(envs[whichfs].env_id, type, fsreq, PTE_D);
...
int fsipc_open(const char *path, u_int omode, struct Fd *fd) {
	...
	req->req_omode = omode;
	return fsipc(FSREQ_OPEN, req, fd, &perm,getFsNumber(path));
}
...
int fsipc_map(u_int fileid, u_int offset, void *dstva) {
	...
	whichfs = FILEID2FSNO(fileid);
	if ((r = fsipc(FSREQ_MAP, req, dstva, &perm,whichfs)) < 0) {
		return r;
	}
...
int fsipc_set_size(u_int fileid, u_int size) {
	...
	req->req_size = size;
	whichfs = FILEID2FSNO(fileid);
	return fsipc(FSREQ_SET_SIZE, req, 0, 0,whichfs);
}
...
int fsipc_close(u_int fileid) {
	...
	u_int whichfs = FILEID2FSNO(fileid);
	req = (struct Fsreq_close *)fsipcbuf;
	req->req_fileid = fileid;
	return fsipc(FSREQ_CLOSE, req, 0, 0,whichfs);
}
...
int fsipc_dirty(u_int fileid, u_int offset) {
	...
	req->req_fileid = fileid;
	req->req_offset = offset;
	return fsipc(FSREQ_DIRTY, req, 0, 0,whichfs);
}
...
int fsipc_remove(const char *path) {
	...
	return fsipc(FSREQ_REMOVE,req,NULL,0,getFsNumber(path));
}
...
int fsipc_sync(void) {
	int r;
	if ((r = fsipc(FSREQ_SYNC,fsipcbuf,0,0,NORMAL_FS_NUMBER)) != 0) {
		return r;
	}
	if ((r = fsipc(FSREQ_SYNC,fsipcbuf,0,0,FAT32_FS_NUMBER)) != 0) {
		return r;
	}
	return 0;
}
