...
u_int addr2disk(void* addr) {
	if ((uint32_t)addr < DISKMAP || (uint32_t)addr >= DISKMAP + DISKMAX) {
		user_panic("asdas");
	}
	return ((uint32_t)addr - DISKMAP) / BY2BLK;
}
...
int walk_path(char *path, struct File **pdir, struct File **pfile, char *lastelem) {
	...
	// start at the root.
	path = skip_slash(path + ROOTDIR_NAMELEN);
	file = &super->s_root;
	dir = 0;
	name[0] = 0;
