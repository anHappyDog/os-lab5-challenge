...
.PHONY: all test tools $(modules) clean run dbg objdump fs-image clean-and-all run-with-fat
...
run-with-fat:
	gxemul -E testmips -C R3000 -M 64  -d 0:target/fs.img -d 1:fat.img target/mos
